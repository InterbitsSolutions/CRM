              ...
              <div class="table-responsive">
                <table id="example23" class="table table-striped table_grid">
                  ...
                  @foreach($bucketFiles as $fileDetails)
                    <tr>
                      <td><a href="{{ url('/bucket_data').DIRECTORY_SEPARATOR.$fileDetails->file_path }}" target="_blank">{{ $fileDetails->file_name }}</a></td>
                      <td>{{ $fileDetails->created_at }}</td>
                      <td>{{ (!empty($fileDetails->modified_at)) ? $fileDetails->modified_at : '-' }}</td>
                      <td>{{ File::size(public_path('bucket_data').DIRECTORY_SEPARATOR.$fileDetails->file_path).' bytes' }}</td>
                    </tr>
                  @endforeach
                  </tbody>
                </table>
