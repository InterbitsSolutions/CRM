      ...
      <div class="row">
        <div class="col-sm-12">
          <div class="white-box">
              ...
              <input type="hidden" name="template_key" value="{{ $templateId }}">
              <div class="form-group">
                ...
                <a href="{{ url('/list-crm-templates') }}"><button type="submit" class="btn btn-inverse waves-effect waves-light m-r-10">Complete Template</button></a>
              ...
              <div class="table-responsive">
                <table id="example23" class="table table-striped table_grid">
                  ...
                  <tbody>
                  @foreach($templateFolders as $folderDetail)
                    <tr>
                  ...
                  @endforeach
                  @foreach($templateFiles as $fileDetails)
                    <tr>
                      <td><a href="{{ url('/template_data').DIRECTORY_SEPARATOR.$templateId.DIRECTORY_SEPARATOR.$fileDetails->file_path }}" target="_blank">{{ $fileDetails->file_name }}</a></td>
                      <td>{{ $fileDetails->created_at }}</td>
                      <td>{{ (!empty($fileDetails->modified_at)) ? $fileDetails->modified_at : '-' }}</td>
                      <td>{{ File::size(public_path('template_data').DIRECTORY_SEPARATOR.$templateId.DIRECTORY_SEPARATOR.$fileDetails->file_path).' bytes' }}</td>
                    </tr>
                  @endforeach
                  </tbody>
                </table>
    ...
    <form name="add_folder" id="add_folder" action="{{ url('/add-folder') }}" method="post">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <input type="hidden" id="templateID" name="template_id" value="{{ $templateId }}">
