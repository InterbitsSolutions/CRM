...
//            flash($xmlResponse,'danger');
    ...
    $getFlag = DB::table('bucket_flag')->where('duplicate_bucket', 'duplicate_bucket')->first();
	$val =$getFlag->flag;
	...
	if(DB::table('bucket_flag')->where('duplicate_bucket', 'duplicate_bucket')->update(array('flag' => 1))){
...
//            }
                    ...
                    if ($existIndex) {
                        try{
                            ...
                            if($counter==$bucketCounter){
								...
								DB::table('bucket_flag')
										->where('duplicate_bucket', 'duplicate_bucket')  
										->update(array('flag' => 0));
                                return json_encode($return);
                            }
                        }
     ...
     */
	  ...
	  public function deleteBucket(){
		   ...
		   $getFlag = DB::table('bucket_flag')->where('delete_bucket', 'delete_bucket')->first();
	       $val =$getFlag->flag;
	if($val == 0){
		//Status Update Code
		if(DB::table('bucket_flag')->where('delete_bucket', 'delete_bucket')->update(array('flag' => 1))){
            ...
            if(!empty($bucketName) && !empty($bucketRegion)){
                try {
                    ...
                    if(empty($regionCode)){
						...
						//Update DB Flag
			 DB::table('bucket_flag')->where('delete_bucket', 'delete_bucket')->update(array('flag' => 0));
                        //return response
                        $return = array(
                            'type' => 'error',
                            'message' => "Region code cannot be empty for $bucketRegion.",
	...
	$message = "Another process is running please wait!Dont refersh browser";
            //return response
            $return = array(
                'value' => '100',
                'type' => 'error',
    ...
    public function deleteMultipleBuckets(){
       ...
       $getFlag = DB::table('bucket_flag')->where('delete_multiple', 'delete_multiple')->first();
	    $val =$getFlag->flag;

        if($val == 0){
	
		if(DB::table('bucket_flag')->where('delete_multiple', 'delete_multiple')->update(array('flag' => 1))){
        if(!empty($_POST)) {
            ...
            if(!empty($buckets)){
                ...
                foreach($buckets as $key => $bucketName){
                    ...
                    if(empty($regionCode)){
						...
						DB::table('bucket_flag')->where('delete_multiple', 'delete_multiple')->update(array('flag' => 0));
                        //return response
                        $return = array(
                            'type' => 'error',
                            'message' => "Region code cannot be empty for $bucketRegion.",
	...
	$message = "Another process is running Please Wait !Dont refresh the browser";
          //return response
        $return = array(
            'value' => '100',
               'type' => 'error',
   ...
   public function createChildBucket()
    ...
    $getFlag = DB::table('bucket_flag')->where('add_bucket', 'add_bucket')->first();
	$val =$getFlag->flag;
	
	if($val == 0){
		//Status Update Code
		if(DB::table('bucket_flag')->where('add_bucket', 'add_bucket')->update(array('flag' => 1))){
...
//            $childBucketLink = "www.support.microsoft$childBucketName.com";
           ...
           // $createBucketResponse = json_decode($this->duplicatorMaster($bucketParams));
            ...
            if($createBucketResponse->type=='success'){
				...
				//Update DB Flag
	 DB::table('bucket_flag')->where('add_bucket', 'add_bucket')->update(array('flag' => 0));
                return json_encode($return);
	 ...
	 DB::table('bucket_flag')->where('add_bucket', 'add_bucket')->update(array('flag' => 0));
	...
	}else{//updating "flag" else start
		$return = array(
                    'type' => 'error',
                    'message' => 'Flag Updation Error',
                );
				return json_encode($return);
		...
		//echo "Already In Processing!Please Wait";
		$return = array(
                    'type' => 'error',
                    'message' => 'Another process is running Please Wait!Dont refresh the Browser!',
                );
